...
在基于自动机理论的模型检测方法中,首先是将抽象出的系统模型用Bǜchi自动机来表示,然后将需要验证的属性用一个LTL公式来描述,并将该公式取反后转化为Bǜchi自动机,最后检查系统自动机的接受语言是否被包含在性质自动机的接受语言中。如果是,则说明此系统具有LTL公式所描述的性质;反之则没有。

有限状态程序是否满足线性规范的自动化检测是近15年来备受关注的问题,线性时态逻辑是一种强大的规范语言，用于表达安全性、活性和公平性。但是模型检测问题被认为是PSPACE-complete 问题，在实践中，模型检查方法面临与复杂性相关的限制:状态空间程序的大小、表示公式的底层自动机的大小，以及应用模型检查算法的产品自动机的大小。人们设计了许多技术来避免状态爆炸问题。通过用二进制决策图表示公式的程序和自动机，可以检查非常大的并发系统。
...
1. 拆分LTL公式
   ...
   如上图的引理所示，首先根据六条规则将LTL公式化为DNF的形式。显然需要在代码中使用递归的想法。但是如何确定应该根据哪一个操作符决定递归规则的使用是一个难点。经过我们仔细的考虑发现几乎所有的情况都可以把LTL公式拆分成左部分，操作符和右部分三个部分。特别地，不满足上述三个部分拆解的原子命题（第1条）或`X`形式公式（第2条）恰好是递归的终止条件，可直接与其分开处理。另外第3，4，5条规则相似，而第6条规则也是终止条件，于是可以进一步地分别进行处理。

   另外，论文中还特意提及`X`形式公式中不能包含子公式的DNF形式。这个时候需要使用分配律进行特殊情况的处理。这一点也在我们的代码实现中有所体现。
2. 建立有向边（转移）
   
   假设$\varphi=(a \land b \land X(\psi)) \lor ...$，则从$\varphi$通过$a \land b$指向$\psi$。以此类推建立从$\varphi$出发的边。
3. 循环操作
   
   另外，如果$\psi$是没有出现过的子公式，则需要进一步对其进行LTL公式的拆解，并根据其生成的DNF形式建立从$\psi$出发的边。同时保持第1步到第3步的循环直至没有新的子公式出现。
4. 建立终止状态

   根据论文4.2的定义，分别根据公式为`Until-Free`还是`Release-Free`分别设定不同的终止状态。假如LTL公式中没有`Until`操作符出现，那么所有生成的状态都是可接收状态；假如LTL公式中没有`Release`操作符出现，那么状态`True`是可接收状态。这一细节的实现可以直接在最开始对LTL公式进行遍历判断其是哪一种类型。当然，特殊的情况是既没有`Release`也没有`Until`出现，那么根据我们对于LTL公式的理解，应该把它归入`Release-Free`一类中，即`True`为最终的可接收状态。
...
2. 第二关键的就是如何建立Buchi自动机，或者说有向图。代码实现如下：
   ...
   ```
    可以比较直观地看出来就是常用的使用队列建立图的方法。其中我们需要判断生成的子公式是否原来出现过，否则会导致代码产生死循环。这一步的实现至关重要，而我们主要根据`X`形式中所包裹的子公式进行排序，然后根据一一对应的关系进行比较。特别地，可以看到我们对于生成的子公式序列进行了清理操作，主要是去掉多余的True和外围的括号。这样就能进一步确保子公式的唯一性。

3. 主要数据结构解析

   我们主要使用`python`中的`dictionary`数据类型保存LTL公式的每个`clause`。

   假设当前$\varphi = (a \land b \land X(c)) \lor ...$。则$(a \land b \land X(c))$就是它的一个`clause`，那么其对应的dict对象为:

   ```python
   [{'atomic': ['a', 'b'], 'xFormula': ['c']}, ...]
   ```

   可以看到，前一部分和后一部分我们对其进行分开存储，这样在应用第6条递归规则的时候就可以较为方便地分开进行合并。并且最外层使用`list`进行包裹，也是为了方便程序实现合并操作。
...
根据论文的结论我们可得，对于`Until/Release-Free`的LTL公式最多有$O(2^{n+1})$个状态，其中$n$为子公式的个数，所以`while`的循环次数是$O(2^{n+1})$。

另外相较于建立有向边，拆分公式的时间复杂度可以忽略不计。而建立有向边的次数与状态数相关，因此总体的时间复杂度为$O(2^{2(n+1)}), 其中n为子公式的个数.$

## 7. 测试

鉴于LTL的递归拆解异常复杂，所以我们的每个`Test Case`都是经过手动推导，再与程序生成的进行比较。所以虽然整体的数量不多，但是我们能够保证其准确性。所以可以用我们的`Test Case`来测试其它程序，相信我们程序的准确率是比较高的。
